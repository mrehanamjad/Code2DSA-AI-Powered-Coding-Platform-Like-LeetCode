  ...
  return (
    <Container>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">My Problem Lists</h1>
      ...
      {lists.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center border rounded-xl bg-muted/20 border-dashed">
          ...
          <h2 className="text-xl font-semibold mb-2 text-foreground">No lists created yet</h2>
          <p className="text-muted-foreground mb-6 max-w-md">
            Create your first problem list to organize your study plan and save questions for later.
          </p>
          <CreateListModal />
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {lists.map((list, idx) => (
            <ListCard key={list._id ? String(list._id) : idx} list={list} />
          ))}
        </div>
      )}
      </div>
